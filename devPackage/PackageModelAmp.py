    ...
    def encode(self):
        ...
        if self.featureDict.get("formula") is True:
            ...
            for sumResult in formulaSequence.descriptor:
                peptideFormula.append(sumResult[0])
            for item in peptideFormula:
                formulaStr = "".join(item)
                formulaStr = re.sub("[A-Z]", "", formulaStr)
                formulaValueList = formulaStr.split()
                allList.append(formulaValueList)
            formulaDf = pd.DataFrame(allList, columns=formulaKeyList)
            formulaDf = formulaDf.fillna(0)
